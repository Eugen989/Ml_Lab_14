...
def p_lab4():
    ...
    if request.method == 'POST':
        X_new = np.array([[float(request.form['list1']),
                           ...
                           float(request.form['list3'])]])
        print(X_new[0])
        predictions = new_neuron.feedforward(X_new[0])
        print("Предсказанные значения:", predictions, np.where(predictions >= 0.5, "Мужчина", "Женщина"))
        return render_template('lab4.html', title="Первый нейрон", menu=menu,
                               class_model="Это: " + str(np.where(predictions >= 0.5, "Мужчина", "Женщина")))
